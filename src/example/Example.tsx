...
const Example = () => {
  ...
  const refreshBalances = useCallback(async () => {
    ...
    setTokenInBalance(await getCurrencyBalance(provider, address, CurrentConfig.tokens.in));
    setTokenOutBalance(await getCurrencyBalance(provider, address, CurrentConfig.tokens.out));
  }, []);
  ...
  return (
    <div className="App">
      ...
      <button onClick={() => wrapETH(100)} disabled={getProvider() === null || CurrentConfig.rpc.mainnet === ""}>
        <p>Wrap ETH</p>
      </button>
      <button
